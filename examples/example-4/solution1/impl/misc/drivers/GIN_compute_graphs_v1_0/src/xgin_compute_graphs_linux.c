...
typedef struct {
    ...
    XGin_compute_graphs_uio_map maps[ MAX_UIO_MAPS ];
} XGin_compute_graphs_uio_info;

/***************** Variable Definitions **************************************/
static XGin_compute_graphs_uio_info uio_info;
...
static int uio_info_read_name(XGin_compute_graphs_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGin_compute_graphs_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGin_compute_graphs_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGin_compute_graphs_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGin_compute_graphs_Initialize(XGin_compute_graphs *InstancePtr, const char* InstanceName) {
	XGin_compute_graphs_uio_info *InfoPtr = &uio_info;
