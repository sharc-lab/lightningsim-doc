...
Description:
 ...
 Input Vectors from Global Memory --->|             |
                               __     | load_inputs |      __
                              |  |<---|_____________|---->|  |
                   in2_stream |  |     _____________      |  | in1_stream
                              |__|--->|             |<----|__|
                                      | compute_add |      __
                                      |_____________|---->|  |
...
// Modified from original to allow in1 and in2 to be in same bundle
static void load_inputs(hls::vector<uint32_t, NUM_WORDS>* in1,
                        hls::vector<uint32_t, NUM_WORDS>* in2,
                        hls::stream<hls::vector<uint32_t, NUM_WORDS> >& in1Stream,
                        hls::stream<hls::vector<uint32_t, NUM_WORDS> >& in2Stream,
                        int vSize) {
mem_rd:
    for (int i = 0; i < vSize; i += 16) {
#pragma HLS LOOP_TRIPCOUNT min = ((c_size + 15) / 16) max = ((c_size + 15) / 16)
        // Load many at a time to enable AXI bursts
        for (int j = 0; j < 16; j++) if (i + j < vSize) in1Stream << in1[i + j];
        for (int j = 0; j < 16; j++) if (i + j < vSize) in2Stream << in2[i + j];
    }
}
...
#pragma HLS INTERFACE m_axi port = in2 bundle = gmem0 depth = c_size
...
#pragma HLS STREAM variable = in1_stream depth = 2  // FIXME: Should this depth be higher?
#pragma HLS STREAM variable = in2_stream depth = 2  // FIXME: Should this depth be higher?
#pragma HLS STREAM variable = out_stream depth = 2  // FIXME: Should this depth be higher?
...
#pragma HLS dataflow
    ...
    load_inputs(in1, in2, in1_stream, in2_stream, vSize);
